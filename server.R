server <- function(input, output, session) {
  ...
  observeEvent(input$LoadDataFile,ignoreInit = T,{
    ...
    updateSelectInput(session, inputId = "FIRE_REGION", choices = c("ALL",fireRegions))
    #updateSelectInput(session, inputId = "FIRE_DISTRICT", choices = c("ALL",fireDistricts))
    updateSelectInput(session, inputId = "DELWP_REGION", choices = c("ALL",delwpRegions))
    updateSelectInput(session, inputId = "FMZ", choices = c("ALL",fireFMZ))
    updateSelectInput(session, inputId = "EFG_NAME", choices = c("ALL",efgNames))
    
    
    seasons <- sort(unique(inData$TFI$SEASON))
    myMin = min(seasons)
    myMax = max(seasons)
    updateSliderInput(session, inputId = "SEASONS", min = myMin,
                      max = myMax,
                      value = c(myMin, myMax))
    updateSliderInput(session, inputId = "BASELINE", min = myMin,
                      max = myMax,
                      value = c(myMin,myMin))
    ...
    rv$TaxonList <- inData$TaxonList %>%
      mutate(VIC_ADVISORY_STATUS = ifelse(is.na(VIC_ADVISORY_STATUS),
                                          "Least Concern",
                                          VIC_ADVISORY_STATUS))
